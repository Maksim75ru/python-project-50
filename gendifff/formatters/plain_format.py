...
def get_stylish(diff: dict, keys=None) -> str:
    lines = []
    parents = ''
    status_key = {
    ...
    for key, val in diff.items():
        ...
        if status == 'nested':
            ...
            if status == 'changed':
                ...
                lines.append(f"{status_key[status].format(path=parents + key, old=old_val, new=new_val)}")
            if status == 'removed':
                lines.append(f"{status_key[status].format(path=parents + key)}")
            if status == 'added':
                work_value = get_complex_value(val.get('value'))
                lines.append(f"{status_key[status].format(path=parents + key, val=work_value)}")
